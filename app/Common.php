...
if (!function_exists('loadView')) {
    function loadView($view, $data = null)
    {
        ...
        if (!isset($data['baseSettings'])) {
            $data['baseSettings'] = Globals::$settings;
        }
        if (!isset($data['generalSettings'])) {
            $data['generalSettings'] = Globals::$generalSettings;
        }
...
if (!function_exists('langBaseUrl')) {
    ...
    {
        if (!empty($route)) {
            return Globals::$langBaseUrl . '/' . ltrim($route, '/');
        }
...
if (!function_exists('trans')) {
    function trans($string)
    {
        ...
        $fallback = [
            'members'        => 'Quản lý Hội Viên (Thêm, Sửa, Xóa, OCR)',
            'industry_types' => 'Quản lý Ngành Nghề Doanh Nghiệp',
        ];
...
if (!function_exists('setPostMetaTags')) {
    ...
    {
        $desc = !empty($post->summary) ? $post->summary : characterLimiter(strip_tags($post->content ?? ''), 160, '...');
        $kw = !empty($post->keywords) ? $post->keywords : generateKeywords($post->title);
        if (!empty($postTags) && is_array($postTags)) {
            $tagList = [];
            foreach ($postTags as $t) {
                if (is_object($t) && !empty($t->tag)) {
                    $tagList[] = $t->tag;
                }
            }
            if (!empty($tagList)) {
        ...
        $data['ogWidth'] = '1200';
        $data['ogHeight'] = '630';
        $data['ogCreator'] = !empty($post->author_username) ? $post->author_username : 'TOP BEST GLOBAL';
        $data['ogAuthor'] = !empty($post->author_username) ? $post->author_username : 'TOP BEST GLOBAL';
        $data['ogPublishedTime'] = !empty($post->created_at) ? date('c', strtotime($post->created_at)) : date('c');
        $data['ogModifiedTime'] = !empty($post->updated_at) ? date('c', strtotime($post->updated_at)) : (!empty($post->created_at) ? date('c', strtotime($post->created_at)) : date('c'));
