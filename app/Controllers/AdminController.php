...
class AdminController extends BaseAdminController
{
    ...
    /**
     * Generate Marketing Email Template Post
     */
    public function newsletterGenerateTemplate()
    {
        checkPermission('newsletter');
        $postsCount = inputPost('posts_count') ?? 4;
        $langId = inputPost('lang_id') ?? $this->activeLang->id;
        
        $templateData = $this->newsletterModel->generateNewsDigestTemplate($postsCount, $langId);
        
        echo json_encode([
            'status' => 'success',
            'subject' => $templateData['subject'],
            'html' => $templateData['html'],
            'posts_count' => $templateData['posts_count'],
