...
class LanguageModel extends BaseModel
{
    ...
    //get language by short form
    public function getLanguageByShortForm($shortForm)
    {
        return $this->builder->where('short_form', cleanSlug($shortForm))->get()->getRow();
    }
    ...
    {
        return $this->builderTranslations->where('lang_id', clrNum($langId))->where('label', cleanStr($label))->get()->getRow();
    }
            ...
            if (!empty($json)) {
                ...
                if (isset($jsonArray->language)) {
                    $data = array(
                        ...
                        'short_form' => isset($jsonArray->language->short_form) ? $jsonArray->language->short_form : 'en',
                        'language_code' => isset($jsonArray->language->language_code) ? $jsonArray->language->language_code : 'en',
                        'text_direction' => isset($jsonArray->language->text_direction) ? $jsonArray->language->text_direction : 'ltr',
                        'text_editor_lang' => isset($jsonArray->language->text_editor_lang) ? $jsonArray->language->text_editor_lang : 'en',
                        'status' => 1,
