<?php
// ==============================================================================
// CHALLENGER 1 ADVERSARIAL STRESS & BOUNDARY TEST SUITE
...
function run_test($name, $closure) {
    ...
    try {
        ...
        echo "  [FAIL] {$name} - Error: " . $e->getMessage() . " in " . basename($e->getFile()) . ":" . $e->getLine() . "\n";
        $totalFailed++;
    }
}

echo "=================================================================\n";
echo "  CHALLENGER 1: ADVERSARIAL & DEEP BOUNDARY SUITE\n";
echo "=================================================================\n\n";
...
// ------------------------------------------------------------------------------
// TEST SUITE 1: 500-Line Limit Compliance Check
// ------------------------------------------------------------------------------
echo "--- 1. File Length Limit (<= 500 lines) Compliance ---\n";
$allFiles = [
...
foreach ($allFiles as $rel) {
    run_test("File {$rel} <= 500 lines", function() use ($rel) {
        $path = FCPATH . $rel;
...
// ------------------------------------------------------------------------------
// TEST SUITE 2: CompanyMatcher 3-Tier Edge & Boundary Tests
// ------------------------------------------------------------------------------
echo "\n--- 2. CompanyMatcher Multi-Tier Stress & Boundary Tests ---\n";
...
run_test("2.2 Short tax codes (<8 chars) reject 100% match tier", function() use ($matcher) {
...
// ------------------------------------------------------------------------------
// TEST SUITE 3: MemberContactModel & MemberBranchModel State Invariants
// ------------------------------------------------------------------------------
...
run_test("3.5 Headquarters branch switching & syncBranches reconcile", function() use ($pdo) {
    ...
    // Reconcile: Make Danang HQ, Add HCMC, Delete Hanoi
    $submitted = [
...
// ------------------------------------------------------------------------------
// TEST SUITE 4: OcrService Multi-Side Merging & Backward Compatibility
// ------------------------------------------------------------------------------
